...
class OddsProvider():
    ...
    def __getAsianOdds(self, p, ec, mid, cid, t1, t2, m):
        aurl = 'http://www.zso8.com/app/midDetailA.asp?a=1&id={0}&cid={1}&t1={2}&t2={3}&Company={4}'.format(mid, cid, t1, t2, ec)
        if test:
            source = self.__getHtml(aurl)
        else:
            source = """
    ...
    def __getOverOdds(self, p, ec, mid, cid, t1, t2, m):
        uurl = 'http://www.zso8.com/app/midDetailU.asp?id={0}&cid={1}&t1={2}&t2={3}&Company={4}'.format(mid, cid, t1, t2, ec)
        if test:
            source = self.__getHtml(uurl)
        else:
            source = """
    ...
    def __getCompanyData(self,p):
        ...
        # 暂时不解析欧赔
        if 0:
            eurl = 'http://www.zso8.com/app/midDetailE.asp?id={0}&cid={1}&t1={2}&t2={3}&Company={4}'.format(mid, cid, t1, t2, ec)
            if 0:
                source = self.__getHtml(eurl)
            else:
                source = """
